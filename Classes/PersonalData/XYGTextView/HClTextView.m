          ...
          maxTextCount:(NSUInteger)maxTextCount
{
    ...
    if (placeText.length > 0) {
        ...
        self.textView.placeholder = [NSString stringWithFormat:@"请输入您的%@, %ld字以内",_leftLabel.text, (unsigned long)_maxTextCount];
    }
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (!position) {
        ...
        _textCountLabel.text = [NSString stringWithFormat:@"%ld/%ld",(unsigned long)textView.text.length,(unsigned long)_maxTextCount];
    }
